...
Production storage now has an S3/R2-compatible driver, image normalization foundation, and listing-scoped duplicate-image content hash detection. Remaining storage hardening includes CDN/cache validation, upload rate-limit review, fraud scoring for cross-listing duplicate images, and broader image moderation validation.
