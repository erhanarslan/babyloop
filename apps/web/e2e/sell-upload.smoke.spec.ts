...
test.describe("sell listing upload flow", () => {
  test("authenticated seller can create a listing and upload an image", async ({ page }) => {
    test.skip(
      !FULL_FLOW_ENABLED,
...
async function loginSellerInBrowser(
  ...
  await page.route("**/api/v1/listings/*/images", async (route) => {
    await fulfillListingImageUpload(route);
  });
...
async function fulfillListingImageUpload(route: Route): Promise<void> {
