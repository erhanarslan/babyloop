export type ListingAgeRange = {
  minMonths: number | null;
  maxMonths: number | null;
};

export const listingAgeRangeOptions = [
...
export function parseListingAgeRangeFormValue(value: string): ListingAgeRange | null {
  const preset = listingAgeRangeOptions.find((option) => option.value === value);
...
export function toListingAgeRangeFormValue(
  minMonths: number | null,
  maxMonths: number | null
): string {
  const preset = listingAgeRangeOptions.find(
...
export function formatListingAgeRange(
  minMonths: number | null,
  maxMonths: number | null
): string {
  const formValue = toListingAgeRangeFormValue(minMonths, maxMonths);
  const preset = listingAgeRangeOptions.find((option) => option.value === formValue);
