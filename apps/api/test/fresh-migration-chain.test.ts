...
describe("fresh database migration chain", () => {
  ...
  it("creates the complete listing_image_review_status enum without unsafe same-transaction enum usage", async () => {
    const result = await client.pool.query<{ enumlabel: string }>(`
      select enumlabel
      from pg_enum
      inner join pg_type on pg_type.oid = pg_enum.enumtypid
      inner join pg_namespace on pg_namespace.oid = pg_type.typnamespace
      where pg_namespace.nspname = 'public'
        and pg_type.typname = 'listing_image_review_status'
      order by pg_enum.enumsortorder
    `);
