...
describe("auth API", () => {
  ...
  it("mobile client login does not return mobile login approval even when mobile approval is enabled", async () => {
    const user = await createUser(app, {
      email: "mobile-client-no-approval@example.com",
      password: "Password123!"
    });
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json().data).not.toHaveProperty("loginApprovalRequired");
    expect(response.json().data).not.toHaveProperty("approvalToken");
    expect(response.json().data.accessToken).toEqual(expect.any(String));
    expect(response.body).not.toContain("passwordHash");
    expect(response.body).not.toContain("password_hash");
  });
